...
import { installRoot, isDevCheckout, checkUpdate, applyUpdate, currentVersion } from "./selfupdate.js";
import { readChangelog, releaseFor } from "./changelog.js";
...
async function buildState() {
  ...
  return {
    version: await currentVersion().catch(() => "unknown"),
...
async function runOnce({ path: raw, name, exclude }, signal) {
  ...
  const size = await getDirSize(snapshotDir);
  const mtime = new Date().toISOString();
  await saveSnapshot(cfg.dest, label, ts, {
...
async function runUpdate({ apply }) {
  ...
  if (await isDevCheckout(root))
    throw new Error("this is a working copy, not an install — update it with git");
  const { staged, changed, from, to } = await checkUpdate();
  if (!changed.length) {
    await rm(staged, { recursive: true, force: true }).catch(() => {});
    logLine(`already up to date — v${from}`, "ok");
    return { changed: [], applied: false, from, to };
  }
  if (!apply) {
    const notes = releaseFor(await readChangelog(staged), to);
    await rm(staged, { recursive: true, force: true }).catch(() => {});
    logLine(`update available — v${from} → v${to} (${changed.length} file(s))`, "warn");
    for (const n of notes?.notes ?? []) logLine("  · " + n);
    return { changed, applied: false, from, to, notes: notes?.notes ?? [] };
  }
  await applyUpdate(staged, root);
  logLine(`updated to v${to} — restart backly web to load it`, "ok");
  return { changed, applied: true, from, to };
}
...
export function startServer({ port = 4849, token = randomBytes(16).toString("hex") } = {}) {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === "/api/changelog")
      return send(res, 200, { releases: await readChangelog(await installRoot()) });
    ...
    if (req.method === "POST") {
      ...
      try {
        ...
        if (route === "update") return send(res, 200, await runUpdate(body));
