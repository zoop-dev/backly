...
async function runUpdate({ apply }) {
  ...
  const { staged, changed } = await checkUpdate();
  if (!changed.length) {
    await rm(staged, { recursive: true, force: true }).catch(() => {});
    logLine("already up to date", "ok");
    return { changed: [], applied: false };
  }
  if (!apply) {
    await rm(staged, { recursive: true, force: true }).catch(() => {});
    logLine(`update available — ${changed.length} file(s) differ`, "warn");
    return { changed, applied: false };
  }
  await applyUpdate(staged, root);
  logLine(`updated ${changed.length} file(s) — restart backly web to load it`, "ok");
